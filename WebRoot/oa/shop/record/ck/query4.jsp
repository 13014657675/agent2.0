	...
	<c:when test="${param.c_libraryid eq 1}">		 	
		  <table width="100%" border="1" cellspacing="0" id="dy">
			...
			<sql:query var="q2" dataSource="${db_mssql }" sql="select * from t_goodsstate  
					where c_orgid=${param.c_orgid } and c_libraryid=${param.c_libraryid } and c_recordid=${param.c_recordid } and c_jobnm='ck' "></sql:query>
			...
			<c:choose>
			<c:when test="${empty q2.rows[0].c_o }">
				<tr>
				<td width="25%" align="center">领用人：</td>
				<td width="25%" align="center">${q2.rows[0].c_p }</td>
				<td width="25%" align="center">时间：</td> 
				<td width="25%" align="center">${q2.rows[0].c_datime} </td>
			</tr>  
			</c:when>
			<c:when test="${not empty q2.rows[0].c_o }">
				<tr>
				<td width="25%" align="center">领用机构：</td>
				<td width="25%" align="center">
				<sql:query var="q5" dataSource="${db_mssql }" sql="select * from t_organization where c_no=${q2.rows[0].c_o }"></sql:query>
				${q5.rows[0].c_name }
				</td>
				<td width="25%" align="center">时间：</td> 
				<td width="25%" align="center">${q2.rows[0].c_datime} </td>
	...
	<c:when test="${param.c_libraryid eq 2 or param.c_libraryid eq 3}">		 	
		  <table width="100%" border="1" cellspacing="0" id="dy">
			...
			<sql:query var="q2" dataSource="${db_mssql }" sql="select * from t_goodsbatch  
					where c_orgid=${param.c_orgid } and c_libraryid=${param.c_libraryid } and c_recordid=${param.c_recordid } and c_jobnm in ('ck')"></sql:query>
			<c:forEach items="${q2.rows }" var="row" varStatus="x">						 
			<tr>
				<td width="25%" align="center">
				<sql:query var="q3" dataSource="${db_mssql }" sql="select * from t_goodstype 
