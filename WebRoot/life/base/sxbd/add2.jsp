...
<html:html lang="true">   
	...
	<script type="text/javascript">
		...
		function back(data)
		{ 
			var str=document.getElementsByName("sxbd.c_tbd")[0].value 
			str = str.replace(/[ ]/g,""); 
			document.getElementsByName("sxbd.c_tbd")[0].value=str 
			if (data.c_tbdl!=str.length)
			{
				alert("投保单号码长度为："+data.c_tbdl)
				document.getElementsByName("sxbd.c_tbd")[0].focus();
			}
		}
		function checktbr()
		{  
	...
	<body onload="settbrq()">
		...
		<table width="100%" border="0" cellspacing="0" align="center" >
				...
				<td colspan="5" ><b style="font-size: 15">投保人资料</b></td>
			 	...
			 	姓名：<nested:text property="c_name" value="" styleId="req" title="投保人姓名必须填写"></nested:text>
			 	</td>
			 	<td>证件类型：
			 	...
			 	<td colspan="2">
			 	证件号码：<nested:text property="c_cardnum" value="" size="40" styleId="req" title="投保人证件号码必须填写" ></nested:text>
			 	</td>
			 	...
			 	<td width="20%"><c:set property="c_sex" value="1" target="${sxbdForm.tbr }"></c:set>
			 	...
			 	<td width="20%"><c:set property="c_marriage" value="0" target="${sxbdForm.tbr }"></c:set>
				...
				<nested:hidden property="c_jobnm" value="add"></nested:hidden> 			
				<tr>
				<c:set property="c_tbrrel" value="父母" target="${sxbdForm.sxbd }"></c:set>
				...
				<tr>
				<td colspan="3">通讯地址：<nested:text property="c_address" value="" size="60" styleId="req" title="通讯地址必须填写"></nested:text> </td>
				<td>邮编：<nested:text property="c_zip" value=""></nested:text></td>
				<td></td>
				</tr>
				<tr>
				...
				<tr >
				<td width="10%"><nested:text property="c_name1" value=""></nested:text> </td>
				<c:set property="c_sex1" value="1" target="${sxbdForm.sxbd }"></c:set>
				<td width="10%"><nested:radio property="c_sex1" value="1"></nested:radio>男
			 		 			<nested:radio property="c_sex1" value="2"></nested:radio>女</td>
				<td width="10%"><nested:select property="c_bbrrel1">
				...
				<td width="10%"><nested:text property="c_sysx1" value="" size="10" onchange="reg_num(this)"></nested:text></td>
				<td width="10%"><nested:text property="c_sybl1" value="" size="10" onchange="reg_num(this)"></nested:text>%</td>
				<td width="10%"><sql:query var="query1"  dataSource="${db_mssql}" sql="select * from t_namecode where c_type=14"></sql:query> 
							 	<nested:select property="c_cardtype1">
							 	<c:forEach var="row" items="${query1.rows}" >		
									<html:option value="${row.c_code }">${row.c_name }</html:option>
								</c:forEach>
							 	</nested:select></td>
				<td width="40%"><nested:text property="c_cardnum1" value="" size="40"></nested:text></td>
				</tr>
				<tr >
				<td width="10%"><nested:text property="c_name2" value=""></nested:text> </td>
				<c:set property="c_sex2" value="1" target="${sxbdForm.sxbd }"></c:set>
				<td width="10%"><nested:radio property="c_sex2" value="1"></nested:radio>男
			 		 			<nested:radio property="c_sex2" value="2"></nested:radio>女</td>
				<td width="10%"><nested:select property="c_bbrrel2">
								<html:option value="本人">本人</html:option>
								<html:option value="配偶">配偶</html:option>
								<html:option value="父母">父母</html:option>
								<html:option value="子女">子女</html:option>
				...
				<td width="10%"><nested:text property="c_sysx2" value="" size="10" onchange="reg_num(this)"></nested:text></td>
				<td width="10%"><nested:text property="c_sybl2" value="" size="10" onchange="reg_num(this)"></nested:text>%</td>
				<td width="10%"><sql:query var="query1"  dataSource="${db_mssql}" sql="select * from t_namecode where c_type=14"></sql:query> 
							 	<nested:select property="c_cardtype2">
							 	<c:forEach var="row" items="${query1.rows}" >		
									<html:option value="${row.c_code }">${row.c_name }</html:option>
								</c:forEach>
							 	</nested:select></td>
				<td width="40%"><nested:text property="c_cardnum2" value="" size="40"></nested:text></td>
				</tr>
				<tr >
				<td width="10%"><nested:text property="c_name3" value=""></nested:text> </td>
				<c:set property="c_sex3" value="1" target="${sxbdForm.sxbd }"></c:set>
				<td width="10%"><nested:radio property="c_sex3" value="1"></nested:radio>男
			 		 			<nested:radio property="c_sex3" value="2"></nested:radio>女</td>
				<td width="10%"><nested:select property="c_bbrrel3">
								<html:option value="本人">本人</html:option>
								<html:option value="配偶">配偶</html:option>
								<html:option value="父母">父母</html:option>
								<html:option value="子女">子女</html:option>
				...
				<td width="10%"><nested:text property="c_sysx3" value="" size="10" onchange="reg_num(this)"></nested:text></td>
				<td width="10%"><nested:text property="c_sybl3" value="" size="10" onchange="reg_num(this)"></nested:text>%</td>
				<td width="10%"><sql:query var="query1"  dataSource="${db_mssql}" sql="select * from t_namecode where c_type=14"></sql:query> 
							 	<nested:select property="c_cardtype3">
							 	<c:forEach var="row" items="${query1.rows}" >		
									<html:option value="${row.c_code }">${row.c_name }</html:option>
								</c:forEach>
							 	</nested:select></td>
				<td width="40%"><nested:text property="c_cardnum3" value="" size="40"></nested:text></td>
				</tr>
				</table>
				</td>				
				</tr>
				...
				<nested:hidden property="c_bd" value=""></nested:hidden>
				<td colspan="2">投保单号：<nested:text property="c_tbd" value="" size="60" styleId="req" title="投保单号必须填写" onchange="check()"></nested:text> </td>
				<td colspan="3">投保日期：<nested:text property="c_tbrq" maxlength="8" value="" styleId="req" title="投保日期必须填写"></nested:text> </td>					
				</tr>
				<tr>
				<td >代理人工号：<nested:text property="c_jobnumber" value="${fn:substring(sessionScope.c_jobnumber,0,2)}" onchange="setname()" styleId="req" title="代理人工号必须填写"></nested:text> </td>
				<td >代理人姓名：<span id="c_name"></span></td>
				<td colspan="3">银行网点：<nested:select property="c_outletsid" styleId="c_outletsid"></nested:select> </td>
				</tr> 	
				<tr>
				<td>银行名称：
				...
				<td colspan="4">银行账号/卡号：<nested:text property="c_bankcode" value="" size="60" styleId="req" title="银行账号/卡号必须填写"></nested:text> </td>
				</tr>
				<tr>
				<c:set property="c_pay" value="12" target="${sxbdForm.sxbd }"></c:set>
				...
				<c:set property="c_pad" value="0" target="${sxbdForm.sxbd }"></c:set>			
				<td>自动垫交：<nested:select property="c_pad">
							<html:option value="0">是</html:option>								
							<html:option value="1">否</html:option>
							</nested:select></td>	
				<c:set property="c_controversy" value="1" target="${sxbdForm.sxbd }"></c:set>			
				...
				<nested:select property="c_productid" >
				...
				function js_bxq()
				{
					var c_bxqtype=document.getElementsByName("sxbd.c_bxqtype")[0].value
				...
				function js_jfq()
				{
					var c_jfqtype=document.getElementsByName("sxbd.c_jfqtype")[0].value
				...
				<td><nested:hidden property="c_jfqx" value="-1"></nested:hidden><nested:text property="c_bxje" value="" size="15" onchange="reg_num(this)"></nested:text></td>
				<td><nested:text property="c_bxf" value="" size="10" onchange="reg_num(this)"></nested:text> </td>
				</tr>
				</table>			
				</td>
				</tr>				
				...
				<tr>
					<td align="center"><input type="button" value=" 添加险种 " onclick="addxz()"></td>
					<td align="center"><html:submit value=" 确 定 " style="font: bold;font-size: 14" /></td>
					<td></td>
					<td align="center"><input type="reset" value=" 重 置 " style="font: bold;font-size: 14"></td>
					<td></td>
				</tr>
			</table>
			<br><br>
