...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../../../tool/styles/main.css">	
		...
		<table width="100%" border="1" cellspacing="0">
				...
				<td align="center" >代理人资格证号</td>
				<td align="center" >证号类型</td>
				<td align="center" >证件证号</td>
				...
				<td align="center" >经理工号</td>
				<td align="center" >经理姓名</td>
				<td align="center" >组育成人工号</td>
				<td align="center" >组育成人姓名</td>
				<td align="center" >总监工号</td>
				<td align="center" >总监姓名</td>
				<td align="center" >部育成人工号</td>
				<td align="center" >部育成人姓名</td>
				</tr>		
				...
				<sql:query var="q1" dataSource="${db_mssql }" sql="select * from t_agentset where c_orgid in (select c_no from t_organization where c_path like '${p2.rows[0].c_path }' and c_flag='0' ) 
					and c_system=1 and c_flag='0' order by c_orgid,c_system,c_type"></sql:query>
				...
				<td align="center" >${a3.rows[0].c_qualificate }</td>
				<sql:query var="a31"  dataSource="${db_mssql}" sql="select * from t_namecode where c_type=14 and c_code=${a3.rows[0].c_certificatetype}"></sql:query>
				<td align="center" >${a31.rows[0].c_name}</td>
				<td align="center" >'${a3.rows[0].c_certificate }</td>
				<td align="center" >${a3.rows[0].c_telephone }[${a3.rows[0].c_mobilephone }]</td>
				...
				<sql:query var="a9"  dataSource="${db_mssql}" sql="exec p_zjbd_1_jl ${row.c_system },'${row.c_jobnumber }',${row.c_type }"></sql:query>
				<td align="center" >${a9.rows[0].c_jobnumber }</td>
				<td align="center" >${a9.rows[0].c_name }</td>
				<sql:query var="a10"  dataSource="${db_mssql}" sql="exec p_zjbd_1_zyc ${row.c_system },'${a9.rows[0].c_jobnumber }'"></sql:query>
				<td align="center" >${a10.rows[0].c_jobnumber }</td>
				<td align="center" >${a10.rows[0].c_name }</td>
				<sql:query var="a11"  dataSource="${db_mssql}" sql="exec p_zjbd_1_zj ${row.c_system },'${row.c_jobnumber }',${row.c_type }"></sql:query>
				<td align="center" >${a11.rows[0].c_jobnumber }</td>
				<td align="center" >${a11.rows[0].c_name }</td>				
				<sql:query var="a12"  dataSource="${db_mssql}" sql="exec p_zjbd_1_byc ${row.c_system },'${a11.rows[0].c_jobnumber }'"></sql:query>
				<td align="center" >${a12.rows[0].c_jobnumber }</td>
				<td align="center" >${a12.rows[0].c_name }</td> 
				</tr>
				</c:forEach>				 
				</table> 
			</body>			
