	...
	<link rel="stylesheet" type="text/css" href="../../../tool/styles/main.css">	
		...
		<table width="100%" border="1" cellspacing="0">
				...
				<sql:query var="q1" sql="select * from t_staff where c_jobnumber='${param.c_jobnumber }'
					and c_orgid in (
						select c_no from  t_organization where c_orgtypeid in (select c_no from t_orgtype where c_order =3)
						union
						select c_no from  t_organization where c_deptid in (
							select c_no from t_dept  where c_ptnature=3002 and c_system=11 )
						union
				...
				<sql:query var="x1" sql="select  * from t_agentset where c_system=11 and c_jobnumber='${row.c_jobnumber }' and c_flag='0'" dataSource="${db_mssql }"></sql:query> 
				...
				<td align="center">
				 		<A href="../../../toModule.do?prefix=/wealth&page=/agent/relation/add4.jsp?c_orgid=${row.c_orgid }&c_jobnumber=${row.c_jobnumber }">添加</A>
				</td>
				</tr>
				</c:if>
				</c:forEach> 
