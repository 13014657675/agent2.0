...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../../../tool/styles/main.css">	
		...
		<table width="100%" border="1" cellspacing="0">
				...
				<tr>
				<td align="center" width="11%">机构类型</td>
				...
				<td align="center" width="11%">离司</td>
				</tr>				
				<c:choose>
				<c:when test="${param.c_type eq 1 }">
				<sql:query var="q1" sql="select * from t_agentset where c_jobnumber='${param.c_jobnumber }' and c_system=11 and c_flag='0' " dataSource="${db_mssql }"></sql:query>
				</c:when> 
				<c:when test="${param.c_type eq 2 }">
				<sql:query var="q1" sql="select  * from t_agentset where c_orgid=${param.c_orgid } and c_system=11 and c_flag='0' order by c_jobnumber " dataSource="${db_mssql }"></sql:query> 
				</c:when>					 			
				</c:choose>					
				...
				<td align="center">
				 		<A href="../../../toModule.do?prefix=/wealth&page=/agent/leave/add4.jsp?c_orgid=${row.c_orgid }&c_jobnumber=${row.c_jobnumber }&c_system=${row.c_system }">离司</A>
				</td>
				</tr>
				</c:forEach> 
